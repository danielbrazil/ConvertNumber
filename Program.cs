...
namespace ConvertNumbers
{
  class Program
  {
    ...
    static void BinaryToOthes()
    {
      ...
      sReceiveNumber = Console.ReadLine();
      Console.WriteLine("-------------------------------------------------------");
      Console.WriteLine("Binary-->Decimal: {0}", ConvertBinaryTo(sReceiveNumber, eConvertNumber.octal));
    ...
    static void DecimalToOthers()
    {
      ...
      if (sReceiveNumber.IndexOf(".") > 0 || sReceiveNumber.IndexOf(",") > 0)
      {
        if (sReceiveNumber.IndexOf(".") > 0)
        {
          dNumber = double.Parse(sReceiveNumber.Substring(0, sReceiveNumber.IndexOf(".")));
          sReceiveNumber = sReceiveNumber.Replace(".", ",");
        }
        else
        {
          dNumber = double.Parse(sReceiveNumber.Substring(0, sReceiveNumber.IndexOf(",")));
        }
        dDecimal = double.Parse(sReceiveNumber);

        Console.WriteLine("-------------------------------------------------------");
        Console.WriteLine("Decimal-->Binary: {0}.{1}",
          ConvertDecimalTo(dNumber, eConvertNumber.binary),
          ConvertDecimalPartTo(dDecimal, eConvertNumber.binary));
        Console.WriteLine("-------------------------------------------------------");
        Console.WriteLine("Decimal-->Octal.: {0}.{1}",
          ConvertDecimalTo(dNumber, eConvertNumber.octal),
          ConvertDecimalPartTo(dDecimal, eConvertNumber.octal));
        Console.WriteLine("-------------------------------------------------------");
        Console.WriteLine("Decimal-->Octal.: {0}.{1}",
          ConvertDecimalTo(dNumber, eConvertNumber.hexa),
          ConvertDecimalPartTo(dDecimal, eConvertNumber.hexa));
      }
      else
      {
        Console.WriteLine("-------------------------------------------------------");
        ...
        Console.WriteLine("-------------------------------------------------------");
        Console.WriteLine("Decimal-->Hexa..: {0}", ConvertDecimalTo(dNumber, eConvertNumber.hexa));
      }
    }
